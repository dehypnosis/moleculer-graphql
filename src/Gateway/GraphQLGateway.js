...
export class GraphQLGateway {
  ...
  handleServiceConnected = async (
    ...
    if (this.connectedServices[serviceName]) {
      ...
      if (!changed && relationships !== currentDefinition.relationships)
        changed = true;
      if (
        !changed &&
        !isEqual(relationDefinitions, currentDefinition.relationDefinitions)
      ) {
        changed = true;
      }
      if (!changed) return;
   ...
   */
  handleServiceDisconnected = async (
    remoteSchemaDef: RemoteSchemaDefinition
  ): Promise<void> => {
        ...
        services
          ...
          .forEach(service => {
            this.handleServiceConnected(
              { ...service.settings, serviceName: service.name },
              true
            );
          });
      }
  ...
  async buildRemoteSchema(
    ...
    this.connectedServices[
      serviceName
    ].remoteExecutableSchema = await createRemoteSchema({
      broker: this.broker,
      service: serviceName
    });
    const computedSchema = parse(schema);
    const definedTypes = computedSchema.definitions
      .filter(d => {
    ...
    this.discoveredGraphQLTypes = concatDifference(
    ...
    if (relationships) {
      ...
      this.requiredGraphQLTypes = concatDifference(
  ...
  getConnectedServiceNames(): Array<GraphQLServiceName> {
    return Object.keys(this.connectedServices).filter(
      serviceName => this.connectedServices[serviceName].remoteExecutableSchema
    );
  }
   ...
   */
  rebuildSchema(): void {
    if (this.schema && !isEmpty(this.connectedServices)) {
      ...
      if (required.length > 0) {
        if (this.broker.logger) {
          this.broker.logger.warn(
            `Schema refresh found ${required.join(
              ", "
            )} types yet to be discovered`
          );
        }
        return;
      }
  ...
  removeExecutableSchema(remoteSchemaDef: RemoteSchemaDefinition) {
    ...
    this.discoveredGraphQLTypes = difference(
      this.discoveredGraphQLTypes,
      definedTypes
    );
  ...
  async start(totalTime: number = 0): Promise<GraphQLSchema> {
    ...
    if (!shouldRetry) {
      ...
      const expected = difference(
        this.settings.expectedServices,
        connectedServices
      );
      if (required.length > 0 || expected.length > 0) {
        ...
        if (this.broker.logger) {
          if (required.length > 0) {
            this.broker.logger.warn(
              `Still waiting for ${required.join(", ")} types to be discovered`
            );
          }
          if (expected.length > 0) {
            this.broker.logger.warn(
              `Still waiting for ${expected.join(
                ", "
    ...
    if (this.broker.logger) {
      this.broker.logger.info(
        `Initial schema generation took ${totalTime +
          computeTimeDiff(startTime)}ms`
      );
    }
